...
## /api/v1/state
### method GET
возвращяет структуру с текущим стейтом по всем каналам. Содержит поля:
...
Пример: 
``D:\git\timerelay\api\app>curl.exe -X GET -H "Content-Type: application/json" http://localhost:8088/api/v1/properties``
...
Все поля опциональны. Если какое-то из них отсутствует, то соответствующая установка остается прежней.
